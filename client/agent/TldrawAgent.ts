...
export class TldrawAgent {
	...
	/** The selected reasoning effort for each model that supports it. */
	$thinkingLevels = atom<AgentThinkingSelections>(
		'thinkingLevels',
		getDefaultThinkingSelections()
	)

	/**
	 * An atom containing the agent's raw JSON history.
	 */
	$jsonHistory = atom<string[]>('jsonHistory', [])

	/** The formatted JSON currently being streamed by the model. */
	$streamingJson = atom<string>('streamingJson', '')

	/** Provider-reported token usage for the last completed request. */
	$lastTokenUsage = atom<AgentTokenUsage | null>('lastTokenUsage', null)
	...
	constructor({ editor, id, onError }: TldrawAgentOptions) {
		...
		// v3 resets model choices saved when Gemini Flash Lite was the default.
		persistAtomInLocalStorage(this.$modelName, `${id}:model-name-v3`, isValidModelName)
		persistAtomInLocalStorage(
			this.$thinkingLevels,
			`${id}:thinking-levels-v1`,
			isThinkingSelectionsRecord
		)
		this.$thinkingLevels.set(normalizeThinkingSelections(this.$thinkingLevels.get()))
		persistAtomInLocalStorage(this.$todoList, `${id}:todo-items`)
		persistAtomInLocalStorage(this.$contextItems, `${id}:context-items`)
		persistAtomInLocalStorage(this.$jsonHistory, `${id}:json-history`)
		persistAtomInLocalStorage(this.$lastTokenUsage, `${id}:last-token-usage`)
		const lastJson = this.$jsonHistory.get().at(-1)
		if (lastJson) this.$streamingJson.set(lastJson)
	...
	/** Get the validated reasoning effort for a model. */
	getThinkingLevel(modelName: AgentModelName): AgentThinkingLevel | null {
		const savedLevel = this.$thinkingLevels.get()[modelName]
		return isThinkingLevelSupported(modelName, savedLevel)
			? savedLevel
			: getDefaultThinkingLevel(modelName)
	}

	/** Save a model-specific reasoning effort. Invalid values reset to the documented default. */
	setThinkingLevel(modelName: AgentModelName, level: unknown) {
		const nextLevel = isThinkingLevelSupported(modelName, level)
			? level
			: getDefaultThinkingLevel(modelName)
		if (!nextLevel) return
		this.$thinkingLevels.update((levels) => ({ ...levels, [modelName]: nextLevel }))
	}
	...
	getFullRequestFromInput(input: AgentInput): AgentRequest {
		...
		const modelName = request.modelName ?? activeRequest?.modelName ?? this.$modelName.get()
		const inheritedThinkingLevel =
			activeRequest && activeRequest.modelName === modelName
				? activeRequest.thinkingLevel
				: this.getThinkingLevel(modelName)
		const requestedThinkingLevel = request.thinkingLevel ?? inheritedThinkingLevel
		const thinkingLevel = isThinkingLevelSupported(modelName, requestedThinkingLevel)
		...
		return {
			...
			modelName,
			thinkingLevel,
			preferHtmlPreview: request.preferHtmlPreview ?? false,
		}
	}
	...
	schedule(input: AgentInput) {
		...
		const modelName = request.modelName ?? scheduledRequest.modelName
		const thinkingLevel =
			request.thinkingLevel ??
			(modelName === scheduledRequest.modelName
				? scheduledRequest.thinkingLevel
				: this.getThinkingLevel(modelName))
		...
		this.setScheduledRequest({
			...
			modelName,
			thinkingLevel,
			preferHtmlPreview: request.preferHtmlPreview ?? scheduledRequest.preferHtmlPreview,
		})
	}
	...
	act(
		...
		options: { recordChatHistory?: boolean } = {}
		...
		if (options.recordChatHistory !== false && util.savesToHistory()) {
	...
	/** Move a wholly off-screen generated drawing into the view used for its request. */
	moveGeneratedShapesIntoView(shapeIds: TLShapeId[], requestBounds: Box) {
		const existingShapeIds = shapeIds.filter((shapeId) => this.editor.getShape(shapeId))
		if (existingShapeIds.length === 0) return

		let generatedBounds: Box | null = null
		for (const shapeId of existingShapeIds) {
			const shapeBounds = this.editor.getShapePageBounds(shapeId)
			if (!shapeBounds) continue
			generatedBounds = generatedBounds ? generatedBounds.union(shapeBounds) : shapeBounds.clone()
		}

		if (!generatedBounds || generatedBounds.collides(requestBounds)) return

		const offset = {
			x: requestBounds.midX - generatedBounds.midX,
			y: requestBounds.midY - generatedBounds.midY,
		}

		this.isActing = true
		try {
			this.editor.run(() => this.editor.nudgeShapes(existingShapeIds, offset), {
				history: 'ignore',
			})
		} finally {
			this.isActing = false
	...
	reset() {
		...
		this.$jsonHistory.set([])
		this.$streamingJson.set('')
		this.$lastTokenUsage.set(null)
...
function requestAgent({ agent, request }: { agent: TldrawAgent; request: AgentRequest }) {
	...
	const generatedShapeIds = new Set<TLShapeId>()
	...
	const requestPromise = (async () => {
		...
		try {
			for await (const action of streamAgent({ agent, prompt, signal })) {
				if (cancelled) break
				editor.run(
					() => {
						...
						if (transformedAction.complete) {
							for (const record of Object.values(diff.added)) {
								if (record.typeName === 'shape') {
									generatedShapeIds.add(record.id as TLShapeId)
								}
							}
						}
			...
			if (!cancelled) {
				agent.moveGeneratedShapesIntoView(
					[...generatedShapeIds],
					Box.From(request.bounds)
				)
			}
		} catch (e) {
				...
				if (match) {
					try {
						const data = JSON.parse(match[1]) as
							| AgentServerSentEvent<AgentAction>
							| { error: string }
						...
						if (isAgentJsonStreamEvent(data)) {
							agent.$streamingJson.set(data.json)
							if (data.complete && data.json) {
								agent.$jsonHistory.update((history) => {
									if (history.at(-1) === data.json) return history
									return [...history, data.json].slice(-10)
								})
							}
							continue
						}

						if (isAgentUsageStreamEvent(data)) {
							agent.$lastTokenUsage.set(data.usage)
							continue
						}

						const { json, ...agentAction } = data
						agent.$streamingJson.set(json)
						yield agentAction
					} catch (err: any) {
...
function isAgentJsonStreamEvent(
	data: AgentServerSentEvent<AgentAction>
): data is AgentJsonStreamEvent {
	return 'type' in data && data.type === 'json' && !('time' in data)
}

function isAgentUsageStreamEvent(
	data: AgentServerSentEvent<AgentAction>
): data is AgentUsageStreamEvent {
	return 'type' in data && data.type === 'usage'
}
 ...
 * This is a helper function that is used internally by the agent.
 */
function persistAtomInLocalStorage<T>(
	atom: Atom<T>,
	key: string,
	...
	try {
		...
		if (stored) {
			...
			if (isValid) {
				if (isValid(value)) atom.set(value)
				else localStorage.removeItem(key)
			} else atom.set(value as T)
		}
	} catch {
...
type AgentThinkingSelections = Partial<Record<AgentModelName, AgentThinkingLevel>>

function getDefaultThinkingSelections(): AgentThinkingSelections {
	const selections: AgentThinkingSelections = {}
	for (const definition of Object.values(AGENT_MODEL_DEFINITIONS)) {
		if (definition.defaultThinkingLevel) {
			selections[definition.name] = definition.defaultThinkingLevel
		}
	}
	return selections
}

function isThinkingSelectionsRecord(value: unknown): value is AgentThinkingSelections {
	return typeof value === 'object' && value !== null && !Array.isArray(value)
}

function normalizeThinkingSelections(value: AgentThinkingSelections): AgentThinkingSelections {
	const normalized = getDefaultThinkingSelections()
	for (const modelName of Object.keys(AGENT_MODEL_DEFINITIONS) as AgentModelName[]) {
		const savedLevel = value[modelName]
		if (isThinkingLevelSupported(modelName, savedLevel)) {
			normalized[modelName] = savedLevel
		}
	}
	return normalized
}
