...
export class ViewportBoundsPartUtil extends PromptPartUtil<ViewportBoundsPart> {
	...
	override buildContent({ userBounds, agentBounds }: ViewportBoundsPart): string[] {
		...
		const response = [
			...
			`Keep every newly created visible shape inside these bounds. Do not generate off-screen coordinates unless the user explicitly asks you to work elsewhere.`,
